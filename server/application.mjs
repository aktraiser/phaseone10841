...
import { AgentVisit } from './agent-visit.mjs';
...
export function createApplication({ databasePath, publicOrigin, trustProxyHops = 0, migrationsDirectory = resolve(root, 'drizzle'), labEnv = process.env, labFactory }) {
  ...
  const laboratory = async request => {
    ...
    if(new URL(request.url).pathname==='/agent/visit'){agentVisit ||= new AgentVisit(lab);return agentVisit.handle(request);}
  ...
  server.once('close', () => { DB.close(); if(lab) server.labShutdown = lab.close({detach:true}).catch(() => console.error('Lab shutdown incomplete; provider timeouts remain active.')); });
  return server;
}
