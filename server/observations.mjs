...
export async function observationRoute(request,DB){
 ...
 if(match){const row=await q('SELECT * FROM observations WHERE id=?',match[1]).first();if(!row)return json({error:'Fiche introuvable'},404);
  ...
  return html(d.title,`<p>${notice}</p><p>Par ${e(d.author)} · déposé le ${new Date(row.created_at).toISOString()}<br>Modèle : ${e(d.model||'Inconnu')} · date de conversation : ${e(d.event_date||'Inconnue')}</p><h2>Contexte</h2><pre>${e(d.context)}</pre><h2>Transcription fournie</h2><pre>${e(d.transcript)}</pre><h2>Analyse du contributeur</h2><pre>${e(d.analysis)}</pre><h2>Captures fournies</h2>${d.images.length?d.images.map((im,i)=>`<figure><img loading="lazy" src="${im.data}" alt="${e(im.caption)}"><figcaption>${i+1}. ${e(im.caption)}</figcaption></figure>`).join(''):'<p>Aucune capture fournie.</p>'}<p><a href="/forum#forum/${row.id}">Réagir dans le forum</a></p><p><a href="/observations/${row.id}.md">Markdown</a> · <a href="/api/observations/${row.id}">JSON</a></p><details><summary>Retirer ma fiche</summary><label>Clé privée de retrait<input id="remove-key" type="password"></label><button id="remove" data-id="${row.id}">Supprimer ma fiche et ses captures</button><p id="status" role="status"></p></details><script src="/observations.js"></script>`);
 }
 if((path==='/observations'||path==='/api/observations')&&['GET','HEAD'].includes(request.method)){
  ...
  return html('Observations humaines',`<p>Des conversations documentées par leurs participants. Ces contributions non vérifiées font partie du forum et peuvent recevoir des réponses.</p><p><a href="/observations/new">+ Documenter une conversation</a></p>${rows.slice(0,20).map(r=>`<p><a href="/observations/${r.id}">${e(r.title)}</a> — ${e(r.author)}</p>`).join('')||'<p>Aucune observation publiée.</p>'}${next!==null?`<a href="?offset=${next}">Suivantes</a>`:''}`);
 }
 return null;
}
