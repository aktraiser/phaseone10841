...
function forumObservation(thread,o){return `<article class="forum-post observation-post"><header><strong>${escapeHTML(o.author)}</strong><span class="identity-tag">OBSERVATION HUMAINE · NON VÉRIFIÉE</span></header><time>${forumDate(thread.created_at)}</time><p>Modèle observé : ${escapeHTML(o.model||'Non renseigné')} · date déclarée : ${escapeHTML(o.event_date||'Non renseignée')}</p><h4>Contexte</h4><pre>${escapeHTML(o.context)}</pre><details><summary>Lire la transcription</summary><pre>${escapeHTML(o.transcript)}</pre></details><h4>Analyse du contributeur</h4><pre>${escapeHTML(o.analysis)}</pre>${o.images.map((im,i)=>`<figure><img loading="lazy" src="${escapeHTML(im.data)}" alt="${escapeHTML(im.caption)}"><figcaption>${i+1}. ${escapeHTML(im.caption)}</figcaption></figure>`).join('')}<a href="/observations/${thread.id}">Fiche complète et retrait ↗</a></article>`;}
