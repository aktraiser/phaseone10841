...
Both visit operations require `X-Visit-Token: VISIT_TOKEN` in addition to the access key. The runtime must keep tokens outside model messages. A restart invalidates live session tokens; the E2B lifetime still applies and reserved quotas are not refunded. Do not automatically retry an uncertain creation.
